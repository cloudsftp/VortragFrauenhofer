...
\newlength{\LevelDistance}\setlength{\LevelDistance}{2em}    % the distance from level to level
...
\begin{tikzpicture}
	[transform shape,scale=\Scale,
		...
		level 1/.style={settextwidth=$\sigma\sigma^{2}$, level distance=4em},
		level 2/.style={settextwidth=$\sigma\sigma^{2}\sigma\sigma$, level distance=5em},
	...
	\path[widthlevel=-1,settextwidth=$\sigma$] node[name=r,fill=yellow] at (0,.5\NodeHeight) {\tiny$\sigma$}
	node[name=s] at (0,-\TreeDiameter-\NodeHeight) {\tiny$\rho$};

	\path[settextwidth=$\sigma\sigma$] node[name=sr] at (1.5 \LevelDistance,-.5\TreeDiameter)
	{\tiny$\sigma\rho$}
	child{ node{\tiny$\sigma^{2}\rho$}
			child{ node[fill=yellow]{\tiny$\sigma^{3}\rho$} }
			child{ node{\tiny$\sigma^{2}\rho\sigma\rho$} }
		}
	%---------%
	child{ node[fill=yellow]{\tiny$\sigma\rho^{2}$}
			child{ node{\tiny$\sigma\rho\sigma\rho^{2}$} }
			child{ node[fill=yellow]{\tiny$\sigma\rho^{3}$} }
		};
	...
	\node[text width=1ex,draw=black!1,very thin] at (1.1,3.2\NodeHeight) {\scriptsize 1};
	\node[text width=1ex,draw=black!1,very thin] at (2.45,3.2\NodeHeight) {\scriptsize 2};
	\node[text width=1ex,draw=black!1,very thin] at (4.2,3.2\NodeHeight) {\scriptsize 3};

	% horizontal line beneath column labels
	\draw[thick,->] (-0.2,2.5\NodeHeight) -- (4.8,2.5\NodeHeight);
