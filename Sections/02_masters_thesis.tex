...
\begin{frame}{Translation of Symbolic Sequences [Type B]}
	\centering
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		\path[draw=none] (0,0) -- (12,2);
		%
		...
		%
		\only<3-5>{
			\node (cycleA) at (a) {$\A^{3}\B^{4}\C^{1}\D^{2}$};
		}
		\only<4-5>{
			\node (cycleB) at (b) {$\L^{3}\R^{4}\L^{1}\R^{2}$};
		}
		\only<5>{
			\node (cycleC) at (c) {$\L^{1}\R^{2}\L^{3}\R^{4}$};
			\draw[double equal sign distance] (cycleB) -- (cycleC) node[midway,right=0.5em,scale=0.5] {$\equiv_{2}$};
		}
		\only<4-5>{
			\draw[->] (cycleA) -- (cycleB) node[midway,right=0.5em,scale=0.5] {$T^{-1}$};
		}
		...
		%
		\only<6-7>{
			\node (cycleA) at (a) {$\L^{1}\R^{2}\L^{3}\R^{4}$};
		}
		\only<7>{
			\node (cycleB) at (b) {$\A^{1}\B^{2}\C^{3}\D^{4}$};
			\draw[->] (cycleA) -- (cycleB) node[midway,right=0.5em,scale=0.5] {$T$};
		...
		\only<9-10>{
			\node (cycleB) at (a_) {$\L^{3}\R^{4}\L^{1}\R^{2}$};
			\draw[double equal sign distance] (cycleA) -- (cycleB) node[midway,right=0.5em,scale=0.5] {$\equiv_{2}$};
		}
		\only<10>{
			\node (cycleC) at (c) {$\A^{3}\B^{4}\C^{1}\D^{2}$};
			\draw[->] (cycleB) -- (cycleC) node[midway,right=0.5em,scale=0.5] {$T$};
		}
		%
	\end{tikzpicture}
\end{frame}
