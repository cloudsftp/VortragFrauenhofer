...
\begin{frame}{Translation: At Most Two Coexisting Cycles}
	...
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		...
		\only<1-4>{
			...
			\node (s4sigma) at (ar) {$s_{2}^{2}\left(\sigma\right)$};
			\draw[->] (sigma) -- (s4sigma) node[midway,above=0.2em,scale=0.5] {$s_{2}^{2}$};
		}
		\only<2-4>{
			\node (phi) at (bl) {$\phi$};
			\node (s4phi) at (br) {$s_{4}\left(\phi\right)$};
...
\begin{frame}{Translation: No Coexistence when Number of 2-Syllables Odd}
	...
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		...
		\only<2-6>{
			...
			\node (s2sigma) at (ar) {$\sigma_{2}\ldots\sigma_{2n+1}\sigma_{1}$};
		}
		\only<3>{
