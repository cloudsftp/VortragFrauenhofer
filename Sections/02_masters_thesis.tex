...
\begin{frame}{Translation of Symbolic Sequences [Type B]}
	...
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		...
		\only<5>{
			\node (cycleC) at (c) {$s_{2}\left(\L^{1}\R^{2}\L^{3}\R^{4}\right)$};
			\draw[double equal sign distance] (cycleB) -- (cycleC);
		}
		\only<4-5>{
			\draw[->] (cycleA) -- (cycleB);
		}
		...
		%
		\only<8-11>{
			\node (cycleA) at (a) {$s_{2}\left(\L^{1}\R^{2}\L^{3}\R^{4}\right)$};
		}
		\only<9-11>{
			\node (cycleB) at (a_) {$\L^{3}\R^{4}\L^{1}\R^{2}$};
			\draw[double equal sign distance] (cycleA) -- (cycleB);
		}
		\only<10>{
