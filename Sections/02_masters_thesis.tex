\sectionframe{First Result: Archetypal Model}
...
\sectionframe{Second Result: Period Adding}
...
\begin{frame}{Translation: Pairs of 2-Syllables}
	\begin{itemize}
		\item 2-Syllable: $\L^{n}\R^{m}$
		\item 4-Syllable: $\A^{a}\B^{b}\C^{c}\D^{d}$
	\end{itemize}
	\vspace{2em}
	\begin{center}
		\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
			\path[draw=none] (0,1) -- (14,-3);
			%
			\coordinate (a) at (3, 0);
			...
			%
			\only<2>{
				\node (cycleA) at (a) {$\L^{a}\R^{b}\L^{c}\R^{d}$};
				\node (cycleB) at (b) {$\A^{a}\B^{b}\C^{c}\D^{d}$};
			}
			\only<3>{
				\node (cycleA) at (a) {$\sigma_{1}\sigma_{2}$};
				\node (cycleB) at (b) {$\phi_{1}$};
			}
			\only<4-5>{
			\node (cycleA) at (a) {$\sigma_{2}\sigma_{3}$};
			\node (cycleB) at (b) {$\phi'_{1}$};
			}
			%
			\only<2-5>{
				\draw[->] (cycleA) -- (cycleB) node[midway,above=0.2em,scale=0.5] {$t$};
			}
			%
		\end{tikzpicture}
	\end{center}
	...
	\only<5>{
		Equivalence:
		\begin{itemize}
			\item Halved: $\sigma \equiv_{2} \rho \Leftrightarrow \sigma = s_{2}^{p}(\rho)$
			\item Full: $\phi \equiv_{4} \psi \Leftrightarrow \phi = s_{4}^{q}(\psi)$
		\end{itemize}
	}
\end{frame}
...
\begin{frame}{Translation: At Most Two Coexisting Cycles}
	...
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		...
		\only<6>{
			\node[scale=0.4] (phi) at (bl) {$t\left(\sigma_{1}\sigma_{2}\right) \ldots t\left(\sigma_{2k+1}\sigma_{1}\right) t\left(\sigma_{2}\sigma_{3}\right) \ldots t\left(\sigma_{2k}\sigma_{2k+1}\right)$};
			\node[scale=0.4] (s4phi) at (br) {$t\left(\sigma_{3}\sigma_{4}\right) \ldots t\left(\sigma_{2k+1}\sigma_{1}\right) t\left(\sigma_{2}\sigma_{3}\right) \ldots t\left(\sigma_{2k}\sigma_{2k+1}\right)t\left(\sigma_{1}\sigma_{2}\right)$};
		}
		\only<7-9>{
...
\begin{frame}{Translation: No Coexistence when Number of 2-Syllables Odd}
	...
	\begin{tikzpicture}[thick,scale=1, every node/.style={font=\huge}]
		...
		\only<3>{
			\node[scale=0.4] (phia) at (bl) {$t\left(\sigma_{1}\sigma_{2}\right) \ldots t\left(\sigma_{2k+1}\sigma_{1}\right) t\left(\sigma_{2}\sigma_{3}\right) \ldots t\left(\sigma_{2k}\sigma_{2k+1}\right)$};
			\node[scale=0.4] (phib) at (br) {$t\left(\sigma_{2}\sigma_{3}\right) \ldots t\left(\sigma_{2k}\sigma_{2k+1}\right) t\left(\sigma_{1}\sigma_{2}\right) \ldots t\left(\sigma_{2k+1}\sigma_{1}\right)$};
		}
		\only<4-7>{
		\node[scale=0.6] (phia) at (bl) {$\phi_{1} \ldots \phi_{k} t(\sigma_{2k+1}\sigma_{1}) \phi'_{1} \ldots \phi'_{k}$};
		\node[scale=0.6] (phib) at (br) {$\phi'_{1} \ldots \phi'_{k} \phi_{1} \ldots \phi_{k} t(\sigma_{2k+1}\sigma_{1})$};
		}
		\only<5>{
