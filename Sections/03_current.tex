...
%	\vspace{-1em}
%	\begin{align*}
%		\phantom{x} \hspace{1em} \text{(i)} \hspace{3em}	 +1 \cdot e^{z_{2b}^{+}} - 1 & = V_{ref}(\tau+ z_{2b}^{+}) + \chi_{c} \hspace{7em} \phantom{x} \\[2em]
%		\only<2-5>{
%		\text{(ii)} \hspace{2.85em} -1 \cdot e^{z_{2b}^{\dagger}} + 1                 & = \left| V_{ref}(\tau + z_{2b}^{\dagger}) + \chi_{c} \right|    \\
%		}
%		\only<3-5>{
%		\Leftrightarrow \hspace{1em} -e^{z_{2b}^{\dagger}} + 1                        & = -\left( V_{ref}(\tau + z_{2b}^{\dagger}) + \chi_{c} \right)   \\
%		}
%		\only<4-5>{
%		\Leftrightarrow \hspace{1.8em}  e^{z_{2b}^{\dagger}} - 1                      & = V_{ref}(\tau + z_{2b}^{\dagger}) + \chi_{c}                   \\[3em]
%		}
%		\only<5>{
%		\Longrightarrow                                                               & \quad z_{2b}^{\dagger}          = z_{2b}^{+}
%		}
%	\end{align*}
%\end{frame}

\begin{frame}{General Proof of Symmetry --- Thesis}
	\vspace{1em}
	Shift symmetrical $\left[f(x + n) = -f(x) \mod 2n\right]$:
	\begin{itemize}
		\item Continuous model
		      \only<2-3>{
		\item Deriving technique of discrete model
		      }
	\end{itemize}
	...
	\only<3>{
		\begin{itemize}
			\item[$\Rightarrow$] Discrete model symmetrical under translation $\left[f(x + n) = f(x) \mod 2n\right]$
		\end{itemize}
	}
\end{frame}

\begin{frame}{Example: Non-symmetrical Deriving Technique}
	\begin{figure}[ht]
		\centering
		\includegraphics[height=0.5 \textheight]{Figs/example_non_symmetrical_deriving.png}
	\end{figure}

	\begin{itemize}
		\item Always counting switching at lower boundary
		      \only<2>{
		\item[$\Rightarrow$] Introduce virtual boundary at $0$ \\
		      Negative half-wave: count switching at upper boundary
		      }
	\end{itemize}
\end{frame}
